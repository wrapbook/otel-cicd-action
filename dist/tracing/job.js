...
async function traceWorkflowRunJobs({ provider, workflowRunJobs, }) {
    ...
    for (const job of workflowRunJobs.jobs) {
        if (job.name.toLowerCase().includes("buildkite")) {
            try {
                ...
                const telemetryAnnotation = annotationsResponse.data.find((a) => a.annotation_level === "notice" &&
                    a.path === "buildkite-telemetry" &&
                    a.message?.includes("buildkite.enabled"));
                if (telemetryAnnotation && telemetryAnnotation.message) {
                    try {
                        buildkiteTelemetry = JSON.parse(telemetryAnnotation.message);
                    }
                    catch (e) {
                        core.debug(`Failed to parse telemetry annotation: ${telemetryAnnotation.message}`);
                    }
                    break;
                }
            }
    ...
    if (Object.keys(buildkiteTelemetry).length === 0) {
        ...
        try {
            ...
            const telemetryCheck = checksResponse.data.check_runs.find((cr) => cr.name === "Buildkite Telemetry");
            if (telemetryCheck) {
                ...
                if (telemetryAnnotation && telemetryAnnotation.message) {
                    try {
                        ...
                        core.debug(`Failed to parse telemetry annotation from check run: ${telemetryAnnotation.message}`);
                    }
                }
            }
        }
    ...
    const rootSpan = tracer.startSpan(workflowRunJobs.workflowRun.name ||
        ...
        attributes: {
            ...
            "github.head_branch": workflowRunJobs.workflowRun.head_branch || undefined,
    ...
    try {
        ...
        for (let i = 0; i < workflowRunJobs.jobs.length; i++) {
            ...
            if (workflowRunJobs.jobs[i].conclusion === "skipped") {
                ...
                const annotations = [];
                // To limit API usage, only get annotations for failed jobs
                if (workflowRunJobs.jobs[i].conclusion === "failure") {
                    const annotationsResponse = await octokit.rest.checks.listAnnotations({
                        owner: github_1.context.repo.owner,
                        repo: github_1.context.repo.repo,
                        check_run_id: job.id,
                    });
                    core.debug(`Found JobAnnotations Count: ${annotationsResponse.data.length}`);
                    ...
                    annotations.push(...annotationsResponse.data.filter((a) => a.annotation_level === "failure" && a.path === ".github"));
                }
